...
SampleAudioOutputFilterDecimate::SampleAudioOutputFilterDecimate(uint8_t taps, const int16_t *tap, int num, int den, SampleAudioOutput *sink)
{
  this->sink = sink;
...
SampleAudioOutputFilterDecimate::~SampleAudioOutputFilterDecimate()
{
  free(hist[1]);
  free(hist[0]);
  free(tap);
...
bool SampleAudioOutputFilterDecimate::SetRate(int hz)
{
...
bool SampleAudioOutputFilterDecimate::SetBitsPerSample(int bits)
{
  return sink->SetBitsPerSample(bits);
}

bool SampleAudioOutputFilterDecimate::SetChannels(int channels)
{
  return sink->SetChannels(channels);
}

bool SampleAudioOutputFilterDecimate::SetGain(float gain)
{
  return sink->SetGain(gain);
}

bool SampleAudioOutputFilterDecimate::begin()
{
  return sink->begin();
}

bool SampleAudioOutputFilterDecimate::ConsumeSample(int16_t sample[2])
{
...
bool SampleAudioOutputFilterDecimate::stop()
{
  return sink->stop();
}
