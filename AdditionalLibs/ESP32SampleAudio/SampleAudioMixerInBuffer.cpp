...
SampleAudioMixerInBuffer::SampleAudioMixerInBuffer(int buffSizeSamples, SampleAudioMixerOutBuffer *dest, int mixChannelNo )
{
...
SampleAudioMixerInBuffer::~SampleAudioMixerInBuffer()
{
  free(leftSample);
  free(rightSample);
...
bool SampleAudioMixerInBuffer::SetRate(int hz)
{
  return sink->SetMixRate( hz, channelNo ); // 44.100
}

bool SampleAudioMixerInBuffer::SetBitsPerSample(int bits) {
  return sink->SetMixBitsPerSample( bits , channelNo); // 16 Bit
}

bool SampleAudioMixerInBuffer::SetChannels(int channels){
  mychannels = channels;
  return sink->SetMixChannels(channels , channelNo); // Stereo or Mono
}

bool SampleAudioMixerInBuffer::begin(){
  filled = false;
...
bool SampleAudioMixerInBuffer::ConsumeSample(int16_t sample[2]){
...
bool SampleAudioMixerInBuffer::stop()
{
